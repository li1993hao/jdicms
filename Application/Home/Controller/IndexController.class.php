...
class IndexController extends HomeController {
     ...
     * @param $p
     */
    public function  category($id,$p=1){
        $id = url_decode($id);
        ...
        if($Category['type']==1){
            ...
            //单页面
            $model_id =  api('Category/get_category',array('id'=>$id,'filed'=>'model_id'));
            ...
            $info =  api('Document/record',array('category'=>$id,'modelName'=>$model_name));
    ...
    public  function content($catid,$id){
        $id = url_decode($id);
        $catid = url_decode($catid);

        $model_id =  api('Category/get_category',array('id'=>$catid,'filed'=>'model_id'));
        if(empty($model_id)){//分类不存在或者被禁用
            $this->error('分类不存在或者被禁用!');
        }
        $model_name = api('Model/get_model_by_id',array('model_id'=>$model_id,'filed'=>'name'));
        ...
        $info =  api('Document/record',array('category'=>$catid,'modelName'=>$model_name,'id'=>$id));
